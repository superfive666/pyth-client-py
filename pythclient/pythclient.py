...
class PythClient:
    def __init__(
        ...
        *,
        ...
        self.solana = solana_client or SolanaClient(
            ...
            ws_endpoint=solana_ws_endpoint,
            client=aiohttp_client_session,
        )
    ...
    def solana_ratelimit(self) -> Union[ratelimit.RateLimit, Literal[False]]:
        # Rate limit being set to "False" literal or 0 or negative to mean unlimited
    ...
    async def refresh_all_prices(self) -> None:
        ...
        # The 3-tuple comprises of the below components:
        # 1: This is the PythProductAccount that the prices correspond to
        # 2: The linked list of PythPriceAccount starting with the root being the first price account product pointing to
        # 3: The current PythPriceAccount to be processed
        tuples: List[
            Tuple[PythProductAccount, List[PythPriceAccount], PythPriceAccount]
        ] = [
            (
                product,
                [],
                PythPriceAccount(
                    product.first_price_account_key, self.solana, product=product
                ),
            )
        ...
        while len(tuples) > 0:
            if account_json is not None:
                ...
                for _, _, price in tuples:
                    ...
                    if p_data is None:
                        raise exceptions.MissingAccountException(
                            f"need account {price.key} but missing in getProgramAccount response"
                        )
                    price.update_with_rpc_response(slot, p_data)
            else:
                await self.solana.update_accounts([price for _, _, price in tuples])

            # Append current pyth price account into the linked list
            # retrieve the next pyth price account key for processing if any:
            # if there is, push into the next processing tuple queue for the next rount
            # else set the linked list of pyth price accounts as the product prices - end for this product
            next_tuples: List[
                Tuple[PythProductAccount, List[PythPriceAccount], PythPriceAccount]
            ] = []
            ...
            # prepare the next iteration of product price refreshing
            tuples = next_tuples
    ...
    async def _refresh_using_program(self) -> Tuple[int, Dict[str, Any]]:
        # get all prices using getProgramAccounts, program key must exist to perform this operation
        assert self._program_key
        resp = await self.solana.get_program_accounts(
            self._program_key, with_context=True
        )
        slot: int = resp["context"]["slot"]
        account_json: Dict[str, Any] = dict(
            (account["pubkey"], account["account"]) for account in resp["value"]
        )
    ...
    async def _refresh_products(
        ...
        *,
        ...
        existing_products = (
            dict((product.key, product) for product in self._products)
            if self._products
            else {}
        )
        ...
        for k in product_account_keys:
            ...
            if account_json is not None:
                ...
                if p_data is None:
                    raise exceptions.MissingAccountException(
                        f"need account {k} but missing in getProgramAccount response"
                    )
                assert slot
    ...
    async def _refresh_mapping_accounts(
        self,
        *,
        account_json: Optional[Dict[str, Any]] = None,
        slot: Optional[int] = None,
    ) -> List[PythMappingAccount]:
        key = self._first_mapping_account_key

        existing_mappings = (
            dict((mapping.key, mapping) for mapping in self._mapping_accounts)
            if self._mapping_accounts
            else {}
        )
        ...
        while key:
            ...
            if account_json is not None:
                ...
                if m_data is None:
                    raise exceptions.MissingAccountException(
                        f"need account {key} but missing in getProgramAccount response"
                    )
                assert slot
    ...
    async def check_mapping_changes(
        self,
    ) -> Tuple[
        List[Union[PythMappingAccount, PythProductAccount]],
        List[Union[PythMappingAccount, PythProductAccount]],
    ]:
        ...
        old_accounts = dict(
            (product.key, product)
            for product in [*self._mapping_accounts, *self._products]
        )
        ...
        return list(new_accounts[key] for key in added_keys), list(
            old_accounts[key] for key in removed_keys
        )
...
class WatchSession:
    ...
    async def reconnect(self):
        ...
        try:
            ...
            logger.debug(
                "connected, resubscribing to {} accounts...", len(resubscribe_accounts)
            )
            self._pending_sub = {}
            ...
            resubscribe_programs.extend(
                (key, self._subid_to_program_accounts[subid])
                for key, subid in self._programkey_to_subid.items()
            )
            logger.debug(
                "connected, resubscribing to {} program accounts...",
                len(resubscribe_programs),
            )
    ...
    async def _program_subscribe(
        self,
        programkey: SolanaPublicKeyOrStr,
        accounts: Iterable[SolanaAccount],
    ...
    def program_subscribe(
        self, programkey: SolanaPublicKeyOrStr, accounts: Iterable[SolanaAccount]
    ):
    ...
    async def program_unsubscribe(self, programkey: SolanaPublicKeyOrStr):
        ...
        try:
            logger.trace(
                "unsubscribing from program {} with subid {}...", keystr, subid
            )
    ...
    def update_program_accounts(
        ...
        accounts_dict = dict((str(account.key), account) for account in accounts)
        self._subid_to_program_accounts[
            self._programkey_to_subid[keystr]
        ] = accounts_dict

    async def next_update(self) -> SolanaAccount:
        """
        This is the main function that create a asynchronous task to poll price update from
        the solana network.
        """
        while True:
            try:
            ...
            if method == "accountNotification":
                ...
                if account:
                    ...
                    # Non-initialized account should be logged and skipped and continue for the next message from the network
                    logger.warning(
                        f"got update for account {account_key} from programSubscribe, but this account was never initialised"
                    )
            else:
                # unknown message will be skipped and ignored and continue to poll the next message from the network
                logger.warning(f"unknown method {method} update from Solana: {msg}")
