...
def test_price_account_update_from(
...
def test_price_account_str(
    ...
    # The to-string method prints the product symbol after the account
...
def test_price_account_agregate_conf_interval(
    ...
    price_account.update_from(buffer=price_account_bytes, version=2, offset=0)
    assert isinstance(price_account.aggregate_price_info, PythPriceInfo)
...
def test_price_account_get_aggregate_price_status_still_trading(
    ...
    price_account.slot = (
        price_account.aggregate_price_info.pub_slot + MAX_SLOT_DIFFERENCE
    )

    assert price_account.aggregate_price_status == PythPriceStatus.TRADING
...
def test_price_account_get_aggregate_price_status_got_stale(
    ...
    # only accept maximum of 25 slot number difference
    price_account.slot = (
        price_account.aggregate_price_info.pub_slot + MAX_SLOT_DIFFERENCE + 1
    )

    assert price_account.aggregate_price_status == PythPriceStatus.UNKNOWN
