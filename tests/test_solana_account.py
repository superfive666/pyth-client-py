...
@pytest.mark.asyncio
async def test_solana_account_update_success(
    solana_account: SolanaAccount, mock_get_account_info: AsyncMock
) -> None:

    mock_get_account_info.return_value = {
        "context": {"slot": 93752509},
        "value": {"lamports": 1000000001},
    }
    ...
    assert (
        ...
        == mock_get_account_info.return_value["value"]["lamports"]
    )


@pytest.mark.asyncio
async def test_solana_account_update_fail(
...
@pytest.mark.asyncio
async def test_solana_account_update_null(
    solana_account: SolanaAccount,
    mock_get_account_info: AsyncMock,
    caplog: LogCaptureFixture,
    solana_pubkey: SolanaPublicKey,
    ...
    exc_message = (
        f"got null value from Solana getAccountInfo for {solana_pubkey}; "
