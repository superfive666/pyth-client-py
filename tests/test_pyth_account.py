...
def product_account(
    ...
    price_account_pubkey: SolanaPublicKey,
) -> PythProductAccount:
...
def test_product_account_update_with_rpc_response_with_data(
    ...
    slot = 96866600
    value = {"lamports": 1000000000, "data": [product_account_b64, "base64"]}
...
def test_pyth_account_update_with_rpc_response_wrong_type(
    pyth_account: PythAccount, product_account_b64: str
) -> None:
    slot = 96866600
    value = {"lamports": 1000000000, "data": [product_account_b64, "base64"]}

    exc_message = (
        f"wrong Pyth account type {PythAccountType.PRODUCT} for {type(pyth_account)}"
    )
...
def test_pyth_account_update_with_rpc_response_no_data(
    pyth_account: PythAccount, caplog: LogCaptureFixture
) -> None:
    slot = 106498726
    value = {"lamports": 1000000000}

    exc_message = (
        f"invalid account data response from Solana for key {pyth_account.key}: {value}"
    )
    with pytest.raises(ValueError, match=exc_message):
        pyth_account.update_with_rpc_response(slot=slot, value=value)
