...
MAPPING_ACCOUNT_B64_DATA = (
    "1MOyoQIAAAABAAAAWAAAAAEAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAABqIGcc"
...
PRICE_ACCOUNT_B64_DATA = (
    "1MOyoQIAAAADAAAAEAsAAAEAAAD3////GwAAAAIAAAAfPsYFAAAAAB4+xgUAAAAA0B+GxYIAAAB/xYqq"
...
def get_account_info_resp(
    ...
    return {"context": {"slot": 96866599}, "value": {"data": [b64_data, "base64"]}}
...
def get_program_accounts_resp() -> Dict[str, Any]:
    return {
        "context": {"slot": 96866599},
        "value": [
            {
                "account": {
                    "data": [MAPPING_ACCOUNT_B64_DATA, "base64"],
                    "executable": False,
                    "lamports": 5143821440,
            ...
            {
                "account": {
                    "data": [PRODUCT_ACCOUNT_B64_DATA, "base64"],
                    "executable": False,
                    "lamports": 4351231,
                    "owner": V2_PROGRAM_KEY,
                    "rentEpoch": 224,
            ...
            {
                "account": {
                    "data": [PRICE_ACCOUNT_B64_DATA, "base64"],
                    "executable": False,
                    "lamports": 23942400,
                    "owner": V2_PROGRAM_KEY,
                    "rentEpoch": 224,
...
def mapping_account(solana_client: SolanaClient) -> PythMappingAccount:
    mapping_account = PythMappingAccount(
        key=SolanaPublicKey(V2_FIRST_MAPPING_ACCOUNT_KEY), solana=solana_client
    )
...
def mapping_account_entries() -> List[SolanaPublicKey]:
    return [SolanaPublicKey(BCH_PRODUCT_ACCOUNT_KEY)]
...
def mock_get_account_info(mocker: MockerFixture) -> AsyncMock:
    ...
    mocker.patch(
        "pythclient.solana.SolanaClient.get_account_info", side_effect=async_mock
    )
    return async_mock
...
def mock_get_program_accounts(mocker: MockerFixture) -> AsyncMock:
    ...
    mocker.patch(
        "pythclient.solana.SolanaClient.get_program_accounts", side_effect=async_mock
    )
    return async_mock
...
def test_get_ratelimit(
    ...
    # check if the RateLimit class rather than literal (unlimited rate)
    assert isinstance(ratelimit, RateLimit)
...
async def test_get_all_accounts(
    ...
    # 0 - PythMappingAccount
    # 1 - PythProductAccount
    # 2 - PythPriceAccount
    assert isinstance(accounts[0], PythMappingAccount)
    assert isinstance(accounts[1], PythProductAccount)
    assert isinstance(accounts[2], PythPriceAccount)
...
def test_create_watch_session(pyth_client: PythClient) -> None:
